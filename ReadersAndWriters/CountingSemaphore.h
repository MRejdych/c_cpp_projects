...
public:
    ...
    void grantResourceForWriter() {
        ...
        while (TRUE) {
            _adjuvantMutex->lock();
            //Warunek przyznania dost�pu pisarzowi: Czytelnia jest pusta oraz nie mamy do czynienia z sytuacja, w ktorej inny pisarz wlasnie opucil czytelnie
            //(drugi warunek jest pomijany, jezeli zaden czytelnik nie oczekuje na dostep)
            if ((_freeResources == _maxResources) &&
                (_writerJustLeftReadingRoom != TRUE || _readersWaitingCounter == 0)) {
                _writersWaitingCounter--;
    ...
    void grantResourceForReader() {
        ...
        while (TRUE) {
            _adjuvantMutex->lock();
            //Warunek przyznania dostepu czytelnikowi: W czytelni znajduje si� miejsce dla czytelnika, nie ma w niej obecnie pisarza oraz pisarz nie czaka w kolejce
            //(ostatni warunek jest pomijany, je�eli flaga _writerJustLeftReadingRoom ma warto�� TRUE)
