...
#include <mutex>
#include <fstream>
/*Klasa odpowiada za gromadzenie i przetwarzanie danych statystycznych dotycz¹cych czasów odczytu i zapisu*/
...
private:
	int _writersAmount;
	int _readersAmount;
	volatile int _savedWritersTimes; //Liczby zapisanych czasów pracy czytelników i pisarzy
	volatile int _savedReadersTimes;
	float* writersWorkTimesData; //Tablice z wynikami czasu pracy czytelników i pisarzy
	float* readersWorkTimesData;
	std::mutex* _adjuvantMutex; //Pomocniczy mutex dbaj¹cy o zachwowanie atomowoœci wykonywanych operacji

	std::fstream file; // Obiekt klasy fstream wykorzystywany do zapisywania wyników do pliku
...
public:
	...
	float getAverageWritingTime() // Oblicza œredni czas zapisu
	{
		float avg
			= 0;
		for (int i = 0; i < _writersAmount; i++)
	...
	float getWritingTimeStandardDeviation() // Oblicza odchylenie standardowe od œredniego czasu zapisu
	{
	...
	float getAverageReadingTime() // Oblicza œredni czas odczytu
	{
		float avg = 0;
		for (int i = 0; i < _readersAmount; i++)
		{
	...
	float getReadingTimeStandardDeviation() // Oblicza odchylenie standardowe od œredniego czasu odczytu
	{
	...
	void saveToFile() {		//Zapisuje do pliku Srednie czas pracy, odchylenia standardowe, czasy maksymalne i minimalne.
