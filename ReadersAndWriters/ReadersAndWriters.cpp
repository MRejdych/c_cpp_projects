...
int main(int argc, char **argv) {
    ...
    StatisticalDataGatherer *_statisticalData = new StatisticalDataGatherer((_writers * _experimentRepeat), (_readers *
                                                                                                             _experimentRepeat)); //Obiekt przechowujacy i przetwarzajacy dane statystyczne
    ...
    switch (arbiterType) { //Wyb�r algorytmu
        case '1':
    ...
    cout << endl << "Sredni czas, ktory watek musial przeznaczyc na odczyt: " << avgReadingTime << " milisekund(y). "
         << endl
...
void waitForThreads(DWORD nCount, const HANDLE *lpHandles, BOOL bWaitAll, DWORD dwMILIseconds) {
    ...
    int i = 0;
    while (i < fullRounds) {
        WaitForMultipleObjects(64, lpHandles + (64 * i), bWaitAll, dwMILIseconds); //limit 64 w�tk�w
        i++;
    }
